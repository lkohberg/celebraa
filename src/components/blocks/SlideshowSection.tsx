...
const SlideshowSection = ({ images, accentColor, lang, blockConfig }: { images?: string[]; accentColor?: string; lang?: EventLang; blockConfig?: any }) => {
  const configImages = blockConfig?.slideshow_images || [];
  const allImages = configImages.length > 0 ? configImages : (images && images.length > 0 ? images : demoImages);
      ...
      <div className="relative max-w-3xl mx-auto px-4">
        ...
        <div className="relative aspect-[16/10] rounded-2xl overflow-hidden shadow-xl">
          <AnimatePresence mode="wait">
            <motion.img key={current} src={allImages[current]} alt="" className="absolute inset-0 w-full h-full object-cover" initial={{ opacity: 0, scale: 1.08 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.8 }} />
          </AnimatePresence>
