...
const PotluckSection = ({ items, accentColor, isPreview = false, lang }: { items?: PotluckItem[]; accentColor?: string; isPreview?: boolean; lang?: EventLang }) => {
      ...
      <div className="relative max-w-xl mx-auto px-4">
        ...
        <div className="space-y-2.5">
          {displayItems.map((item, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }} className="flex items-center justify-between p-4 bg-background/80 backdrop-blur-sm rounded-xl border border-border/50 hover:shadow-sm transition-all">
              <div className="flex items-center gap-3">
                ...
                <span className={`font-body text-sm ${item.claimedBy ? "text-muted-foreground line-through" : "text-foreground font-medium"}`}>{item.name}</span>
              </div>
              {item.claimedBy ? (
                <span className="font-body text-xs text-muted-foreground bg-secondary px-2.5 py-1 rounded-full">{item.claimedBy}</span>
              ) : (
                <Button size="sm" variant="outline" className="font-body text-xs h-8 rounded-lg" disabled={isPreview} onClick={() => { if (isPreview) return; const updated = [...displayItems]; updated[i] = { ...updated[i], claimedBy: l("potluckYou") }; setDisplayItems(updated); }}>
                  {l("potluckClaim")}
                </Button>
              )}
            </motion.div>
