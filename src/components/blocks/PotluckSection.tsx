      ...
      <div className="relative max-w-xl mx-auto px-4">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-6 md:mb-10">
          <div className="inline-flex items-center justify-center w-12 h-12 md:w-14 md:h-14 rounded-full mb-3" style={{ backgroundColor: colorWithAlpha(color, 0.15) }}>
            <ShoppingBasket className="w-5 h-5 md:w-6 md:h-6" style={{ color }} />
          </div>
          <h2 className="font-display text-xl md:text-3xl text-foreground">{l("potluck")}</h2>
          ...
          <p className="font-body text-sm text-muted-foreground mt-2">{l("potluckSubtitle")}</p>
        </motion.div>
        ...
        <div className="space-y-2">
          {normalized.map((item, i) => {
            ...
            return (
              <motion.div key={i} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }} className="flex flex-col p-3 md:p-4 bg-background/80 backdrop-blur-sm rounded-xl border border-border/50 hover:shadow-sm transition-all">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2.5">
                    {isClaimed ? (
                      <div className="w-5 h-5 md:w-6 md:h-6 rounded-full flex items-center justify-center bg-green-100"><Check className="w-3 h-3 text-green-600" /></div>
                    ) : (
                      <div className="w-5 h-5 md:w-6 md:h-6 rounded-full border-2 border-border" />
                    )}
                    <span className={`font-body text-sm ${isClaimed ? "text-muted-foreground line-through" : "text-foreground font-medium"}`}>{item.name}</span>
                  </div>
                  {isClaimed ? (
                    <span className="font-body text-xs text-muted-foreground bg-secondary px-2 py-0.5 rounded-full">{claimedByName}</span>
                  ) : (
                    <Button size="sm" variant="outline" className="font-body text-xs h-7 md:h-8 rounded-lg" disabled={isPreview || claimMutation.isPending} onClick={() => handleClaim(item.name, i)}>
