...
const VideoMessageSection = ({ accentColor, lang, blockConfig, variant = "wedding" }: VideoMessageSectionProps) => {
  ...
  return (
    <section className="py-8 pb-20 md:py-20 md:pb-36 relative overflow-hidden bg-card">
      ...
      <div className="relative max-w-2xl mx-auto px-4 text-center">
        ...
        >
          <div className="inline-flex items-center justify-center w-10 h-10 md:w-14 md:h-14 rounded-full mb-2" style={{ backgroundColor: colorWithAlpha(color, 0.15) }}>
            {mediaType === "audio" ? (
              <Mic className="w-4 h-4 md:w-6 md:h-6" style={{ color }} />
            ) : (
              <Video className="w-4 h-4 md:w-6 md:h-6" style={{ color }} />
            )}
          </div>
          <h2 className="font-display text-lg md:text-3xl text-foreground mb-0.5">{label}</h2>
          <p className="font-body text-xs md:text-sm text-muted-foreground mb-4 md:mb-8">{subtitle}</p>

          {mediaUrl ? (
            mediaType === "audio" ? (
              <div className="bg-background/60 backdrop-blur-sm rounded-2xl border border-border/30 p-3 md:p-6">
            ...
            <div className="rounded-2xl overflow-hidden border border-border/30 shadow-sm">
              <div className="aspect-video bg-gradient-to-br from-secondary via-card to-secondary flex items-center justify-center relative">
                ...
                <div className="text-center relative">
                  <div className="w-14 h-14 md:w-20 md:h-20 mx-auto mb-2 rounded-full flex items-center justify-center" style={{ backgroundColor: colorWithAlpha(color, 0.1) }}>
                    {isCorporate ? (
                      <Video className="w-6 h-6 md:w-8 md:h-8" style={{ color, opacity: 0.5 }} />
                    ) : (
                      <Heart className="w-6 h-6 md:w-8 md:h-8" style={{ color, opacity: 0.5 }} />
                    )}
                  </div>
