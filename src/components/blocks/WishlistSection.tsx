    ...
    <section className="py-20 relative overflow-hidden">
      ...
      <div className="relative max-w-2xl mx-auto px-4">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-12">
          ...
          <h2 className="font-display text-2xl md:text-3xl text-foreground">{l("wishlist")}</h2>
          ...
          <p className="font-body text-sm text-muted-foreground mt-3 max-w-md mx-auto">{l("wishlistSubtitle")}</p>
        </motion.div>

        <div className="space-y-3">
          {displayItems.map((item, i) => (
            <motion.div key={i} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="group flex items-center justify-between p-5 bg-card/80 backdrop-blur-sm rounded-xl border border-border/50 hover:border-border hover:shadow-md transition-all duration-300">
              <div className="flex items-start gap-4">
                <div className="w-10 h-10 rounded-lg flex items-center justify-center shrink-0 mt-0.5" style={{ backgroundColor: colorWithAlpha(color, 0.1) }}>
