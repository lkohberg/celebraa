      ...
      <div className="relative max-w-2xl mx-auto px-4">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-6 md:mb-12">
          <div className="inline-flex items-center justify-center w-12 h-12 md:w-14 md:h-14 rounded-full mb-3" style={{ backgroundColor: colorWithAlpha(color, 0.15) }}>
            <Gift className="w-5 h-5 md:w-6 md:h-6" style={{ color }} />
          </div>
          <h2 className="font-display text-xl md:text-3xl text-foreground">{l("wishlist")}</h2>
          ...
          <p className="font-body text-sm text-muted-foreground mt-2 max-w-md mx-auto">{l("wishlistSubtitle")}</p>
        </motion.div>

        <div className="space-y-2.5">
          {displayItems.map((item, i) => (
            <motion.div key={i} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="group flex items-center justify-between p-3.5 md:p-5 bg-card/80 backdrop-blur-sm rounded-xl border border-border/50 hover:border-border hover:shadow-md transition-all duration-300">
              <div className="flex items-start gap-3">
                <div className="w-8 h-8 md:w-10 md:h-10 rounded-lg flex items-center justify-center shrink-0 mt-0.5" style={{ backgroundColor: colorWithAlpha(color, 0.1) }}>
                  <Gift className="w-3.5 h-3.5 md:w-4 md:h-4" style={{ color, opacity: 0.6 }} />
                </div>
                <div>
                  <p className="font-body font-medium text-sm text-foreground">{item.name}</p>
                  {(item.note || item.hint) && <p className="font-body text-xs text-muted-foreground mt-0.5">{item.note || item.hint}</p>}
                </div>
              </div>
              {item.url && (
