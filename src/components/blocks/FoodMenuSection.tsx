      ...
      <div className="relative max-w-2xl mx-auto px-4">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-8 md:mb-14">
          <div className="inline-flex items-center justify-center w-12 h-12 md:w-14 md:h-14 rounded-full mb-3" style={{ backgroundColor: colorWithAlpha(color, 0.15) }}>
            <UtensilsCrossed className="w-5 h-5 md:w-6 md:h-6" style={{ color }} />
          </div>
          <h2 className="font-display text-xl md:text-3xl text-foreground">{l("menu")}</h2>
        ...
        <div className="bg-card/60 backdrop-blur-sm rounded-2xl border border-border/40 p-5 md:p-12 shadow-sm">
          <div className="space-y-6 md:space-y-10">
            {displayMenu.map((item, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.15 }} className="text-center">
                {item.course && <p className="font-body text-[10px] tracking-[0.3em] uppercase mb-2" style={{ color }}>{item.course}</p>}
                <h3 className="font-display text-lg md:text-2xl text-foreground mb-1">{item.name}</h3>
                {item.description && <p className="font-body text-xs md:text-sm text-muted-foreground italic">{item.description}</p>}
