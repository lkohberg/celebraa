...
const demoGames: GameOption[] = [
  { name: "Beer Pong", emoji: "🍺", votes: 12 },
  { name: "Karaoke", emoji: "🎤", votes: 8 },
  { name: "Flunkyball", emoji: "⚽", votes: 15 },
  { name: "Wer bin ich?", emoji: "🤔", votes: 6 },
];
    ...
    <section className="py-20 relative overflow-hidden">
      ...
      <div className="relative max-w-xl mx-auto px-4">
        ...
        >
          ...
          <p className="font-body text-sm text-muted-foreground mt-3">Stimme ab, welche Spiele wir spielen!</p>
        </motion.div>
        ...
        <div className="space-y-3">
          {displayGames.map((game, i) => (
            <motion.div
              ...
              className="relative bg-background/80 backdrop-blur-sm rounded-xl border border-border/50 overflow-hidden hover:shadow-sm transition-shadow"
            >
              <motion.div
                className="absolute inset-y-0 left-0 opacity-10"
                style={{ backgroundColor: color }}
                initial={{ width: 0 }}
                whileInView={{ width: `${(game.votes / maxVotes) * 100}%` }}
                viewport={{ once: true }}
                ...
                <div className="flex items-center gap-3">
                  ...
                  {!voted && (
                    <Button
                      size="sm"
                      variant="outline"
                      className="font-body text-xs h-8 px-3 rounded-lg"
                      onClick={() => {
                        if (isPreview) return;
