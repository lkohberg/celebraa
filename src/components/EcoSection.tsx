          ...
          <div className="flex flex-wrap justify-center gap-4">
            {badges.map((badge, i) => (
              <motion.div
                ...
                className="flex items-center gap-2 bg-background border border-emerald-200 dark:border-emerald-800 rounded-full px-5 py-2.5"
              >
                <badge.icon className="w-4 h-4 text-emerald-600" />
                <span className="font-body text-sm font-medium text-foreground">{badge.label}</span>
              </motion.div>
            ))}
          </div>
        </motion.div>
