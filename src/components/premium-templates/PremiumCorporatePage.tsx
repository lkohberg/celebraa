...
import { Building2, MapPin, Clock, Calendar, Shirt } from "lucide-react";
...
const PremiumCorporatePage = ({ event, theme, lang }: { event: PremiumEventData; theme?: PremiumTheme; lang?: EventLang }) => {
  ...
  return (
    <div className="min-h-screen" style={{ fontFamily: theme?.font ? `'${theme.font}', sans-serif` : "'DM Sans', sans-serif" }}>
      ...
      {/* Hero */}
      <section className="relative min-h-[80vh] flex items-center justify-center" style={{
        ...
        <motion.div className="relative z-10 text-center px-4 max-w-3xl" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
          ...
          <h1 className="font-display text-4xl md:text-6xl font-bold text-white mb-6">{event.title}</h1>
          ...
          <div className="flex items-center justify-center gap-6 text-white/70 font-body text-sm">
            ...
            <span className="flex items-center gap-2"><Clock className="w-4 h-4" /> {event.event_time} Uhr</span>
          </div>
      ...
      {event.story_text && (
        <section className="py-24 bg-background">
          <div className="max-w-2xl mx-auto px-4 text-center">
            <h2 className="font-display text-2xl md:text-3xl text-foreground mb-8">{el?.agenda || t("event.agenda")}</h2>
